...
(function () {
  ...
  const SHEETS_API_URL = "/api/reservas";
  ...
  function confirmReservation() {
    ...
    // Mensaje WhatsApp (incluye todos los horarios + links Calendar)
    const header =
...
Horarios:`;
    ...
    const calText = calLinks.map(x => `Clase ${x.i}: ${x.link}`).join("\n");

    const tail =
`\n\nGoogle Calendar (1 link por clase):
${calText}
...
Gracias!`;
    ...
    // Abrir una pestaña con links clickeables (mejor que abrir 10 popups)
...
<head>
<meta charset="utf-8" />
<title>Agregar clases a Google Calendar</title>
<meta name="viewport" content="width=device-width, initial-scale=1" />
<style>
  body{font-family:system-ui,Segoe UI,Arial; padding:16px; line-height:1.4;}
  h2{margin:0 0 8px;}
  .muted{opacity:.75; margin:0 0 14px;}
  a{display:block; padding:10px 12px; margin:8px 0; border:1px solid #ccc; border-radius:10px; text-decoration:none;}
  a:hover{background:#f3f3f3}
  code{background:#f5f5f5; padding:2px 6px; border-radius:6px;}
</style>
</head>
<body>
  <h2>Google Calendar • ${state.nombre} ${state.apellido}</h2>
  <p class="muted">Hacé click en cada clase para agregarla al calendario. (Sí, Google no deja crear 10 eventos con un solo link 😅)</p>
  ${calLinks.map(x => {
    const d = parseISODate(x.s.dateISO);
    const label = `${WEEKDAYS[d.getDay()]} ${x.s.dateISO} ${x.s.start}-${x.s.end}`;
    return `<a href="${x.link}" target="_blank" rel="noopener">➕ Agregar Clase ${x.i}: <b>${label}</b></a>`;
  }).join("")}
  <p class="muted">Tip: si te aparece "Permitir popups", aceptalo para que se abran más fácil.</p>
</body>
</html>`;
      win.document.open();
    ...
    alert("✅ Listo: se abrió WhatsApp y una pestaña con los links para Google Calendar.");
